...
public class MenuService extends TransactionAssistant {
	...
	// this.tranManager.tranStart();
	// // storeCode가 존재한다면 oracle count값 = 1
	// if (this.convertToBoolean(this.sqlSession.selectOne("isStCode",
	// storeMenu))) {
	// storeMenu.setMenuList(this.sqlSession.selectOne("selMenuList",
	// storeMenu));
	// } else {
	// // storeCode가 존재하지 않는다면 oracle count값 = 0
	// storeMenu.setMessage("사업자번호가 조회되지않았습니다.");
	...
	private void modifyMenuInfo(Model model) {
		...
		try {
			...
			if (menuCode != null) {
				...
				// if
				// (this.convertToBoolean(this.sqlSession.insert("insMenuCode",
				// store))) {
				// 메뉴코드 우선추가 ('1998033036', M00, '00000','00000')
				this.tranManager.commit();
	...
	//// MenuBean menu = null;
	// String folderPath =
	// "C:\\Users\\user\\git\\postluck\\postluck\\src\\main\\webapp\\resources\\image\\"
	// + store.getStoreCode() + "\\";
	// String filePath = folderPath;
	...
	// store.getMenuList().get(0).getMenuCode());
